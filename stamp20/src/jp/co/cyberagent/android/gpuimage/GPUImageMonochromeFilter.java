...
/**
 * Converts the image to a single-color version, based on the luminance of each
 * pixel intensity: The degree to which the specific color replaces the normal
 * image color (0.0 - 1.0, with 1.0 as the default) color: The color to use as
 * the basis for the effect, with (0.6, 0.45, 0.3, 1.0) as the default.
 */
public class GPUImageMonochromeFilter extends GPUImageFilter {
    ...
    public void onInit() {
        ...
        mIntensityLocation = GLES20.glGetUniformLocation(getProgram(),
                "intensity");
        mFilterColorLocation = GLES20.glGetUniformLocation(getProgram(),
                "filterColor");
    }
