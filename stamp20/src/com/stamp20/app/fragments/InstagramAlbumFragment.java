...
import com.stamp20.app.imageloader.MyImageView;
import com.stamp20.app.imageloader.ChildAdapter.ViewHolder;
import com.stamp20.app.imageloader.MyImageView.OnMeasureListener;
import com.stamp20.app.instagram.InstagramTokenKeeper;
import com.stamp20.app.instagram.InstagramUtils;
...
//				instagramClient.setAccessToken(new Token(InstagramTokenKeeper.readAccessToken(mContext),null));
		  ...
		  protected void onPostExecute(UserInfoData result){
			if (result == null){
			  Toast.makeText(mContext, "Instagram Authorization required", Toast.LENGTH_LONG).show();
			  //InstagramTokenKeeper.clear(mContext);
			  updateView(false);
			}else{
		  ...
		  protected UserInfoData doInBackground(Void... params) {
			...
			try{
				String token = instagramClient.getAccessToken().getToken();
				Log.i("wangpeng", "token: " + token);
