...
public class StampGLSurfaceView extends GLSurfaceView implements GLSurfaceView.Renderer{
	...
	private int deltaW = 3;

	private int deltaH = 3;
	...
	public void onDrawFrame(GL10 gl) {
		...
		if (currentfilterID != 0 && currentfilterID != 1 ) {
	...
	public void generateStamp(GL10 mGL) {
        ...
        Bitmap frameBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.background_stamp_h_transparent_pierced);
        //deltaW为白色边框的条宽度
        ...
        mGL.glReadPixels((surfaceWidth - mWidth)/2 + deltaW, (surfaceHeight-mHeight)/2 + deltaH, mWidth-2*deltaW, mHeight-2*deltaH, GL_RGBA, GL_UNSIGNED_BYTE, ib);
        int[] ia = ib.array();
        ...
        Bitmap bitmap = Bitmap.createBitmap(frameBitmap.getWidth(), frameBitmap.getHeight(), Config.ARGB_8888);  
        try {  
